...
public class ByteArray implements AutoCloseable {
    ...
    public ByteArray(long arrayLength) {
        if (arrayLength < 0) {
        ...
        if (arrayLength == 0) {
            this.segment = arena.allocate(ValueLayout.JAVA_BYTE, 0);
        } else {
            this.segment = arena.allocate(ValueLayout.JAVA_BYTE, arrayLength);
        }
        clear();
    }
    ...
    public static void main(String[] args) {
        try (ByteArray a = new ByteArray((long) Math.pow(2d, 36d))) {
